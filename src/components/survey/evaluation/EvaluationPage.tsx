...
const EvaluationPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    console.log("isEvaluationLoading: ", isEvaluationLoading);
  }, [isStatusLoading, isEvaluationLoading]);

  // Check if survey is completed and setup redirect timer if needed
  useEffect(() => {
    if (!isLoading) {
      const shouldRedirect = !sessionData;
  ...
  useEffect(() => {
    if (countdown === 0 && showRedirectPopup) {
      ...
      console.log("showRedirectPopup: ", showRedirectPopup);
      router.push("/survey");
    } else {
      console.log("countdown: ", countdown + "s");
      console.log("showRedirectPopup: ", showRedirectPopup);
  ...
  if (isLoading) {
    return <LoadingState message="Sedang memuat data evaluasi..." />;
  }

  if (isComplete && !showRedirectPopup && !isLoading) {
